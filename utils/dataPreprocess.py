		...
		with open(filename, 'r') as f:
			for num in tqdm(range(number)):
				...
				if use_chars:
					...
					question_chars = []
				# ans/cand --> index
				ans_indices = list(map(lambda w: w_dict.get(w, 0), ans.split()))
				candidate_indices = [list(map(lambda w: w_dict.get(w, 0), c)) for c in candidate]
				all_file_information.append((paragraph_indices, question_indices, ans_indices, candidate_indices, \
							paragraph_chars, question_chars, cloze, str(num), ans, candidate_all))
